...
// Splits the secret `key` into `n` keyshares with a treshold of `k`.
...
func CreateKeyshares(key []byte, n int, k int) ([][]byte, error) {
	...
	go_keyshares := make([][]byte, n)
	for i := 0; i < n; i += 1 {
		go_keyshares[i] = keyshares[i*C.sizeof_sss_Keyshare : (i+1)*C.sizeof_sss_Keyshare]
	}

	return go_keyshares, nil
}

// Restores the key as split by `CreateShares`, using the keyshares given in
// `go_keyshares`.
// `go_keyshares` must be a slice of keyshare slices, each of which must be
...
func CombineKeyshares(go_keyshares [][]byte) ([]byte, error) {
	k := len(go_keyshares)
	...
	for i, keyshare := range go_keyshares {
